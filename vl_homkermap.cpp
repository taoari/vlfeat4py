        ...
        double period)
{
    ...
    /* Input */
    // kernel -> kernelType
    m_assert(kernel == "kchi2" || kernel == "kinters" || kernel == "kjs",
            "KERNEL must be one of KCHI2, KINTERS, KJS");
    if (kernel == "kinters")
        kernelType = VlHomogeneousKernelIntersection;
    else if (kernel == "kjs")
        kernelType = VlHomogeneousKernelJS;
    // period
    m_assert(period == -1 || period > 0,
            "PERIOD is not positive");
    // gamma
    m_assert(gamma > 0, "GAMMA is not positive");
    // window -> windowType
    m_assert(window == "uniform" || window == "rectangular",
            "WINDOW must be one of UNIFORM, RECTANGULAR");
    if (window == "uniform")
        windowType = VlHomogeneousKernelMapWindowUniform;
    
    int numDimensions = 2; // TODO: only support matrix now
    int dimensions[2];
    dimensions[0] = XX.n_rows;
    dimensions[1] = XX.n_cols;
