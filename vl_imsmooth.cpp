...
(T * outputImage,
 ...
 int flags)
{
  T * tempImage = (T*) malloc (sizeof(T) * numRows * numOutputColumns) ;
  vl_uindex k ;
  vl_index j ;
  ...
  */
  ...
  switch (kernel) {
    case GAUSSIAN :
    {
      ...
      T * filter = (T*) malloc (sizeof(T) * (2 * W + 1)) ;
      T acc = 0 ;
...
{
    /* Input */
    // padding_ -> padding -> flags
    int padding = VL_PAD_BY_CONTINUITY ;
    m_assert(padding_ == "continuity" || padding_ == "zero",
        "PADDING must be either ZERO or CONTINUITY");
    if (padding_ == "zero") {
        padding = VL_PAD_BY_ZERO ;
    }
    int flags = padding | VL_TRANSPOSE;
    // kernel_ -> kernel
    int kernel = GAUSSIAN ;
    m_assert(kernel_ == "gaussian" || kernel_ == "triangular",
        "KERNEL must be either GAUSSIAN or TRIANGULAR");
  ...
  if (verbose) {
    ...
    printf("vl_imsmooth: [%dx%dx%d] -> [%dx%dx%d] (%s, subsampling step %d)\n",
              N, M, K, N_, M_, K, classid_str, step) ;
    printf("vl_imsmooth: padding: %s\n", padding_str) ;
    printf("vl_imsmooth: kernel: %s\n", kernel_str) ;
    printf("vl_imsmooth: sigma: %g\n", sigma) ;
    printf("vl_imsmooth: SIMD enabled: %s\n",
              vl_get_simd_enabled() ? "yes" : "no") ;
  }

    _vl_imsmooth_smooth_f ((float*) Is.memptr(),
                         M_, N_,
                         (float const*) I.memptr(),
                         M, N, K,
        ...
        int verbose)
{
  ...
  if (verbose) {
    ...
    _vl_imsmooth_smooth_d ((double*) Is.memptr(),
                         M_, N_,
                         (double const*) I.memptr(),
                         M, N, K,
                         kernel, sigma, step, flags) ;
